...
import { Users, FileText, Stethoscope, QrCode, Search, Clock, AlertCircle, UserCheck, Camera, Activity, Heart, TrendingUp, UserPlus } from 'lucide-react';
...
const DoctorDashboard = () => {
  ...
  const loadFamilyInsights = async () => {
    ...
    try {
      const response = await apiClient.getFamilyInsightsOverview();
      if (response.status === 'success' && response.data) {
        setFamilyInsights(response.data);
      } else {
        // Fallback to mock data if no real data available
        const mockFamilyInsights = [
          {
            familyGroupName: "Smith Family",
            totalMembers: 4,
            commonConditions: ["Hypertension", "Diabetes Type 2"],
            riskFactors: ["Genetic predisposition to cardiovascular disease"],
            recentActivity: "3 members shared blood pressure readings",
            memberAgeRanges: { "18-30": 1, "31-50": 2, "51+": 1 },
            sharedRecords: 12
          },
          {
            familyGroupName: "Johnson Family", 
            totalMembers: 3,
            commonConditions: ["Asthma", "Allergies"],
            riskFactors: ["Family history of respiratory conditions"],
            recentActivity: "Child added allergy information",
            memberAgeRanges: { "18-30": 0, "31-50": 2, "51+": 1 },
            sharedRecords: 8
          }
        ];
        setFamilyInsights(mockFamilyInsights);
      }
    } catch (error) {
      console.error('Error loading family insights:', error);
  ...
  return (
    <Layout title="Doctor Dashboard">
      ...
      {/* Family Health Insights Section */}
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="p-6 border-b">
          ...
          <p className="text-sm text-gray-600 mt-1">
            Family health patterns and insights for better patient care
          </p>
        </div>
        ...
        <div className="p-6">
          {loadingInsights ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 mb-2">No family insights available</p>
              <p className="text-sm text-gray-400">
                Family health insights will appear here when patients share family medical data
              </p>
            </div>
          ) : (
                ...
                <div key={index} className="border rounded-lg p-5 hover:shadow-md transition-shadow">
                  ...
                  <div className="mb-4">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-sm font-medium text-gray-700">Common Conditions</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {insight.commonConditions.map((condition: string, idx: number) => (
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {insight.riskFactors.map((risk: string, idx: number) => (
                  ...
                  <div className="mb-4">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-sm font-medium text-gray-700">Age Distribution</span>
                    </div>
                    <div className="flex gap-4 text-xs">
                      <span className="flex items-center">
                        <div className="w-3 h-3 bg-green-500 rounded-full mr-1"></div>
                        18-30: {insight.memberAgeRanges["18-30"]}
                      </span>
                      <span className="flex items-center">
                        <div className="w-3 h-3 bg-blue-500 rounded-full mr-1"></div>
                        31-50: {insight.memberAgeRanges["31-50"]}
                      </span>
                      <span className="flex items-center">
                        <div className="w-3 h-3 bg-purple-500 rounded-full mr-1"></div>
                        51+: {insight.memberAgeRanges["51+"]}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-gray-500">
                        {insight.sharedRecords} shared records
                      </span>
                    </div>
                    <p className="text-xs text-gray-600 mt-1">{insight.recentActivity}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <h4 className="font-medium text-gray-900">Family Insights</h4>
          <p className="text-sm text-gray-600 mt-1">View family health patterns</p>
        </button>
