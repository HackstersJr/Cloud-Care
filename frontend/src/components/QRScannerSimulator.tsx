...
const QRScannerSimulator: React.FC<QRScannerSimulatorProps> = ({ isOpen, onClose }) => {
  ...
  // Sample facility QR codes for testing (these are healthcare facilities, not patients)
  ...
  const simulateScan = (facilityData: any) => {
    setIsScanning(true);
    setScanResult('Scanning facility QR code...');
    
    setTimeout(() => {
      setScanResult('Facility verified! Preparing to share your data...');
      
      setTimeout(() => {
        onClose();
        // Navigate to a share confirmation page instead of showing their data
        navigate('/share-confirmation', { 
          state: { 
            facility: facilityData,
            action: 'share_my_data' // This indicates we're sharing MY data with THEM
          } 
        });
      }, 1000);
    }, 1500);
  ...
  const handleTextInput = () => {
    if (scanResult.trim()) {
      try {
        ...
        // Check if it's a healthcare facility QR code
        if (facilityData.type === 'HEALTHCARE_FACILITY') {
          navigate('/share-confirmation', { 
            state: { 
              facility: facilityData,
              action: 'share_my_data'
            } 
          });
        } else {
          alert('Please scan a healthcare facility QR code to share your data');
        }
        onClose();
      } catch (error) {
